...
- **✅ SHIPPED — Stage 3 (v0.14.0), sandbox/mock only (`FEATURE_CARDCOM_LIVE=false`, אין טרמינל אמיתי):**
  ...
  - **סכמה (mig 034, additive):** `billing_documents`; `payment_transactions += coupon_code/referral_source (D-B7 inert)/kind`; `users += dunning_next_retry_at`; `subscription_events` CHECK מורחב (+5 event-types). **go-live** (credentials + מודול חשבוניות + סוג-מסמך של רו"ח + `FEATURE_CARDCOM_LIVE=true` + Railway cron) = משימה ידנית של נדב, מפורטת ב-SESSION_HANDOFF.
