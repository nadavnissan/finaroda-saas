...
6. **מנוי וחיוב:** trial 14 יום **ללא כרטיס** (ללא tokenization בהרשמה, **ללא חיוב אוטומטי**; בסוף התקופה בחירה אקטיבית של המשתמש — פלאן בתשלום או Free). חיוב מתחדש חל **רק לאחר** המרה אקטיבית לתשלום ולכידת כרטיס (דרך Stripe Checkout; הכרטיס לא נוגע בשרתי החברה). מעבד התשלום הוא **Stripe** (Stripe Checkout + Stripe Billing); ביטול = end-of-period (`cancel_at_period_end`, גישה עד תום התקופה). מסלול **Free** חינמי קבוע. **מסמך המס הישראלי** לכל חיוב מונפק ע"י ספק החשבוניות המוגדר של החברה (‏`INVOICE_PROVIDER`), ולא ע"י Stripe; חשבוניות Stripe אינן מסמך מס ישראלי. מדיניות ביטול/החזר (לעו"ד — חוק הגנת הצרכן, זכות ביטול עסקה). _(change order — נדב 2026-07-09; PSP switched to Stripe, Stage 3R, 2026-07-14.)_
...
6. **צד ג':** Stripe (מעבד התשלום: Stripe Checkout + Stripe Billing), ספק החשבוניות הישראלי המוגדר (`INVOICE_PROVIDER`, מנפיק את מסמך המס הישראלי; טרם נבחר: Green Invoice / iCount / EZcount), Resend (מייל), Cloudflare R2, Sentry — לפרט במדיניות.
