...
## [STAGE 8 / Hardening + PWA + dev→main release] — 2026-07-20
- GOAL: The final pre-100 stage — production hardening, an installable-app PWA baseline, and the authorized `dev`→`main` merge cutting the **v1.0.0** release. This is the ONE run where touching `main` was founder-authorized (via the Stage-8 prompt), and only after the full suite + a fresh production build were green.
- SOLUTION:
  - **A. Housekeeping.**
    - **A1 — deleted `require_active_trial`** (backend/core/auth.py): confirmed dead (zero imports anywhere; trial expiry is already handled by the daily cron → Free + `effective_tier`). Straight deletion per the DEBT-1 founder ruling; no behaviour change, all imports still used elsewhere.
    - **A2 — em-dash JSX-comment lint fix verified** (already shipped in Validation V1): `test_content_copy.py` strips `/* */` (incl. JSX `{/* */}`) and `//` comments before scanning, with a meta-test proving it. No action needed.
    - **A3 — dropped `notifications_legacy_outbox`** via new **migration 038**: the dead mig-005 scheduled-outbox table (renamed aside by mig 031). Confirmed zero rows on dev + zero application reads/writes (only the mig-031 rename referenced it) + not in `test_smoke.EXPECTED_TABLES`. STOP S3 not triggered. Idempotent `DROP TABLE IF EXISTS`.
    - **A4 — removed `peek_token.py`** (a dev-only DB magic-link-token dumper). The other untracked working files (design/, validation/*.docx, _v3/_v4.txt, OPEN_ITEMS) are the founder's materials, untracked → never shipped; left in place (not deleted).
  - **B. RUN_LOCAL.md** — exact Windows PowerShell steps for a fresh clone: venv + activate + `pip install -r backend/requirements.txt`, `backend/.env` from the example with the four values that matter (JWT_SECRET, ENCRYPTION_KEY, DEV_RETURN_MAGIC_LINK=true, FEATURE_PUBLIC_SIGNUPS_OPEN=true) + secret-generation one-liners, `uvicorn backend.main:app --reload --port 8000`, `pnpm install` (repo-root workspace) + `pnpm dev` in frontend/, adding a beta_allowlist user (SQL one-liner), and the Clear-site-data recovery note (the founder lost hours to exactly these).
  - **C. PWA baseline** — installable app, NOT an offline product. Added `app/manifest.ts` (→ static `/manifest.webmanifest`: name/short_name, start_url=/scan, display=standalone, brand theme/background #0b0d12), real PNG icons (192/512 + maskable-512 + apple-touch-180 + 32) generated from an "F" monogram (brand green #1FB286 on #0b0d12), root-layout metadata (manifest link, icons, appleWebApp, themeColor, viewportFit). A **minimal service worker** (`public/sw.js`, registered in production only) serves a graceful `/offline.html` on failed navigations and **never caches app HTML/JS/CSS** (a deliberate choice — no stale-version bugs). No push, no background sync, no offline data.
  - **D. Production config hardening.**
    - **D1 — env templates:** `backend/.env.production.example` (Railway, full commented list — JWT_SECRET, ENCRYPTION_KEY, RESEND_API_KEY, EMAIL_*, CRON_SECRET, STRIPE_* as test keys with FEATURE_STRIPE_LIVE=false, SENTRY DSNs, INVOICE_PROVIDER=mock, DATABASE_URL, DEV_RETURN_MAGIC_LINK=false) + `frontend/.env.production.example` (Vercel — NEXT_PUBLIC_API_URL, NEXT_PUBLIC_FEATURE_ARENA=false, NEXT_PUBLIC_SENTRY_DSN).
    - **D1/D2 — startup guard consolidated + TESTED:** the inline `main.py` DEV_RETURN_MAGIC_LINK check became `config.assert_production_safety()`, a pure, argument-injectable function that refuses to boot a **production** process when `DEV_RETURN_MAGIC_LINK=true` OR `JWT_SECRET` is the dev sentinel. Called at import in `main.py`. 7 new tests (test_production_guard.py) assert every branch. **Guarded flags reported:** DEV_RETURN_MAGIC_LINK + the dev JWT sentinel. **Deliberately NOT guarded** (legitimate production states): FEATURE_STRIPE_LIVE=false (soft-launch), FEATURE_PUBLIC_SIGNUPS_OPEN=true (public launch), FEATURE_ARENA, empty ENCRYPTION_KEY (unused by any code path today).
    - **D3 — CORS/BASE_URL/API_URL wiring:** the existing env-driven CORS (main.py) is already production-correct; documented the domain scheme (repo convention **finaroda.com**): frontend apex `https://finaroda.com` canonical (www 308-redirects at the edge), backend `https://api.finaroda.com`, ADMIN_URL = the frontend origin (admin is a route in the same app). Values set in the env templates. No code change needed.
  - **E. Cron documentation** — a "CRON WIRING" runbook section in SESSION_HANDOFF for the two X-Cron-Secret HTTP endpoints (`/api/cron/notifications` daily ~07:00 UTC; `/api/cron/billing` daily ~01:00 UTC) + reconciled **railway.toml**: removed the broken `subscription-renewal` cron (its script was deleted in Stage 3R — it would have crash-looped) and the redundant native `expire-trials`/`trial-ending-soon` crons (folded into the billing + notifications HTTP endpoints), keeping only the native `resolve-scenarios` job (no HTTP endpoint; must precede the notifications sweep).
  - **F. Release.** Full suite green on the final state; a fresh `pnpm build` verified (22 routes, static 22/22); merged `dev`→`main`; tagged **v1.0.0**.
- FILES CREATED: backend/migrations/038_drop_notifications_legacy_outbox.py, backend/tests/test_production_guard.py, backend/.env.production.example, frontend/.env.production.example, RUN_LOCAL.md, frontend/src/app/manifest.ts, frontend/src/components/app/ServiceWorkerRegister.tsx, frontend/public/sw.js, frontend/public/offline.html, frontend/public/icons/{icon-192,icon-512,maskable-512,icon-32}.png, frontend/public/apple-touch-icon.png.
- FILES MODIFIED: backend/core/auth.py (delete require_active_trial), backend/config.py (DEV_JWT_SENTINEL const + assert_production_safety), backend/main.py (call the guard), railway.toml (cron reconcile), frontend/src/app/layout.tsx (PWA metadata + SW register), frontend/src/lib/version.ts (APP_VERSION 1.0.0), ATP.md, CHANGELOG.md, VERSIONS.md, SESSION_HANDOFF.md.
- DB CHANGES: migration 038 (DROP notifications_legacy_outbox). No data loss (empty, unreferenced).
- CONFIG ADDED: no NEW runtime vars. Production env templates document existing vars with production-safe values; the CRITICAL production guard now enforces DEV_RETURN_MAGIC_LINK=false + a real JWT_SECRET when ENVIRONMENT=production.
- VALIDATION: pytest **215/215** (+7 production-guard) · tsc clean · eslint clean · frontend unit **92/92** · shared **14/14** · `pnpm build` OK (22 routes, static 22/22). Red-line suites untouched/green. Green on dev AND on main after the merge.
...
- SOLUTION:
  - **FX4 backend.** `coin_access` table (mig 037): `plan` PK (free/basic/pro) + `coins` JSON (base symbols) + `wildcard` flag. Seed: free=[LINK,AVAX], basic=[LINK,AVAX,SOL,ADA,DOGE], pro=wildcard. `core/coin_access.py`: `resolve_coin_access` (DB-first, per-request, no cache), `effective_plan` (trial→Pro, legacy advanced→basic), `coin_allowed`/`gated_coins` (**universe-only rule**: only `SCAN_UNIVERSE_BASE` coins are identity-gated; unknown symbols pass so the count-gate law stays orthogonal), `unlock_plan`. Enforcement in `record_scan` after the COUNT gate, before the daily cap → **403 COIN_GATED** `{code,message,blocked,plan}`. New `GET /api/scan/coin-access` returns `{plan,coins,wildcard,universe,locked}` (locked = base→unlock-plan). Entitlements payload shape UNTOUCHED (served separately) so the red-line entitlements law holds.
  - **FX4 admin.** `api/admin_coin_access.py` (require_admin, audited to admin_events): `GET /api/admin/coin-access` (all plans + universe), `PUT /api/admin/coin-access/{plan}` (coins/wildcard, rejects non-universe coins). Frontend `CoinAccessAdmin.tsx` (per-plan checklist + Pro wildcard toggle), registered as the "coin access" console section. Changes take effect WITHOUT a deploy (read per request; TC proves it live).
  - **FX4 frontend.** `lib/scan/coinAccess.ts` (fetch + `isCoinLocked`/`coinLockReason`). Selector shows locked coins with 🔒 + "Available on <plan>", tap routes to /subscribe (never toggles/errors); default selection skips locked coins; `recordScan` also handles a COIN_GATED rejection → /subscribe (edge case). History renders now-gated coins (gating is new-scans only).
  - **F16.** `lib/scan/narrative.ts` — pure `resolveNarrative(input, data)` (first match wins; order S4 pass, then S1 spike, S3 flicker, S5 watch, S2 fallback) + `resolveDailyLimit` (S6). Reads only payload fields (B3): passLabel, score, ema7SlopePct, riskReward, change24h (already computed), whyNotCheckId. **S3 DEGRADED**: no reversal/short-EMA-reclaim flag exists → positive ema7 slope is the proxy. Date-rotated variants (UTC dayOfYear % eligible), missing-placeholder variants skipped. `market_narratives.json` (root + frontend byte-identical) — 6 states × 2-3 DRAFT variants, disclaimer appended, `_source_ref` governance, `[[id|display]]` concept markers. `components/scan/MarketNarrative.tsx` card after ring/list before Blueprint (in Results + EmptyState); S6 renders INSIDE the 429/limit screen (B2). Free = paid.
  - **Founder ruling (red-line vs seed).** The FX4 free-seed gates BTC on free, but `test_v1_redline_reveal` (TC-V1-RVL-03/04) and `test_v1_redline_xp` scanned BTC on a free user (incidental coin). Per founder ruling, swapped the incidental coin BTCUSDT→LINKUSDT (an allowlisted free coin) in those two suites; every reveal/XP law and assertion preserved verbatim. Also swapped the incidental coin in 4 non-red-line scan tests (p2_scan/p2_scorer/pkg_b_phase2) to allowlisted coins.
- FILES CREATED: backend/migrations/037_coin_access.py, backend/core/coin_access.py, backend/api/admin_coin_access.py, backend/tests/test_fx4_coin_gating.py, frontend/src/lib/scan/coinAccess.ts, frontend/src/components/admin/CoinAccessAdmin.tsx, frontend/src/lib/scan/narrative.ts, market_narratives.json, frontend/src/lib/scan/market_narratives.json, frontend/src/components/scan/MarketNarrative.tsx, frontend/tests/narrative.unit.test.ts.
- FILES MODIFIED: backend/api/scan.py (coin-access endpoint + COIN_GATED enforcement), backend/models/scan.py (CoinAccessResponse), backend/main.py (router), backend/tests/test_content_copy.py (narrative governance), backend/tests/{test_v1_redline_reveal,test_v1_redline_xp,test_p2_scan,test_p2_scorer,test_pkg_b_phase2}.py (coin swaps), frontend/src/app/(scan)/scan/page.tsx (locks + narrative + S6), frontend/src/components/scan/Results.tsx (card in Results/EmptyState), frontend/src/lib/scan/persist.ts (COIN_GATED outcome), frontend/src/app/(admin)/admin/page.tsx (section), frontend/src/lib/version.ts.
- DB CHANGES: mig 037 (coin_access table + seed). No changes to existing tables.
- CONFIG ADDED: אין (coin access is DB rows, admin-editable; no env).
- VALIDATION: pytest 203/203 (+18), tsc clean, eslint clean, frontend unit 73/73 (+11), shared 14/14. Red-line suites green.
...
## [FIX-ROUND-FOUNDER / Pre-Stage-8 validation findings] - 2026-07-15
- GOAL: Fix the P0+P1 findings from the founder's manual validation of local dev (v0.17.1). P0: onboarding never ran for a new user; post-scan dead end / 429 not surfacing; scan-detail page broken + forbidden trade words. P1: coin gating (investigate), Settings slow + no billing entry, layout anchor. P2: hamburger unread hint. Content-quality / product-iteration items explicitly deferred.
- SOLUTION:
  - **FX1 (P0) — onboarding routing.** ROOT CAUSE: F13 onboarding was built as a pre-signup anonymous flow with signup *inside* it (S5), but `/login` (magic-link + DEV SIGN-IN) and `/verify` routed straight to `/scan`, so a user who signed in through the login page (incl. an allowlist-inserted user) never entered onboarding — no post-auth guard read `onboarding_completed`. FIX: new `routeAfterAuth()` (`lib/app/session.ts`) reads `/api/auth/me.onboarding_completed` and sends an unfinished user to `/onboarding`, wired into `login` DEV SIGN-IN + `verify`; plus a `/scan` mount safety-net guard for direct navigation. Completion still grants 300 XP once (server-authoritative, idempotent — unchanged).
  - **FX2 (P0) — no dead end after the Free scan.** The dedicated `429` limit screen already existed and the server already returns `429 DAILY_SCAN_LIMIT` on a Free user's 2nd same-day scan (tested); the real gap was NAVIGATION back to the scan input. FIX: added an explicit **"Scan"** entry (first) to the hamburger drawer (`NavDrawer`); the FINARODA wordmark already routes to `/scan`. So a user is never stranded: logo or hamburger -> scan -> 2nd scan -> the 429/quota screen (SEE PLANS / VIEW JOURNAL).
  - **FX3 (P0) — scan-detail page rebuilt to spec.** The Recent-scans detail used the FORBIDDEN words `ENTRY` / `SL` / `TP` as level labels and had no chart. FIX (per PRD §3.5.1 / F3b, confirmed against the docs): replaced with the canonical calculator terminology — **Mathematical Trigger Point / Calculated Risk Level / Dynamic Risk Level / Calculated Target Level** (each with its formula-transparency note), a `Risk:Reward` line, a `TIMING VERIFIED`/`WATCH · score/100` header, and the `BlueprintChart` (Chart Standard v1, layers gated by plan like the live card). These are setup-time levels that were already shown at scan time (NOT withheld outcomes), so redisplay is reveal-gating-safe; journal outcomes/R stay on the F3 dashboard. Chart re-pulls the current market (historical candles are not stored) with the levels drawn as logged.
  - **FX4 (P1) — coin-identity gating: INVESTIGATED, NOT IMPLEMENTED (per rule).** Per-plan coin-IDENTITY gating (which specific coins a plan may scan, e.g. BTC = Pro) is **NOT specified** anywhere in PRD/SPEC/UX — only coin COUNT (Free 2 / Basic 5 / Pro 10) is. E2 ("coins beyond the validated universe") is explicitly V2. Per FX4's rule I did NOT invent the mapping; reported for founder definition. No code.
  - **FX5 (P1) — Settings billing + load.** (a) Added a **"PLAN & BILLING"** card: current plan, TRIAL day/total, a button to `/subscribe`, and the existing cancel-at-period-end action (ChurnSurvey). (b) Load: the profile + prefs fetches were serialized behind `/me` (a 3-wave waterfall). They now fire in parallel (self-authenticating; a 401 still redirects to /login). Profiled the endpoints — `/api/profile` + `/api/notifications/prefs` are single, indexed queries (xp_events/user_settings both indexed); no N+1 or missing index. The waterfall was the cause.
  - **FX6 (P1) — app-shell layout anchor.** The reading screens (scan/dashboard/settings/profile/academy/history) now pin their max-width column with `minHeight:100vh` so short screens fill the viewport (Disclaimer anchored bottom) instead of collapsing "small and unprofessional"; reading columns standardized to `maxWidth 480` (academy stays 1040 for its grid). Academy filter row switched from `overflowX:auto` to `flexWrap:wrap` so it is never cut off. Structural anchor only (full visual polish deferred).
  - **FX7 (P2) — hamburger unread hint.** A green dot on the ≡ icon when unread notifications exist (reuses the bell's server-authoritative unread count); clears when the bell panel opens via a decoupled `finaroda:notifications-read` window event.
- FILES MODIFIED: frontend/src/lib/app/session.ts (routeAfterAuth), app/(auth)/login/page.tsx, app/verify/page.tsx, app/(scan)/scan/page.tsx (guard), components/scan/NavDrawer.tsx (Scan item), components/scan/AppHeader.tsx (unread dot + NOTIF_READ_EVENT), components/app/NotificationBell.tsx (dispatch event), app/(scan)/history/page.tsx (FX3 rebuild), app/(profile)/settings/page.tsx (billing card + parallel fetch + shell), app/(profile)/profile/page.tsx (shell), app/(dashboard)/dashboard/page.tsx (shell), app/(academy)/academy/page.tsx (filter wrap + shell), frontend/src/lib/version.ts; backend/tests/test_content_copy.py (+ forbidden-term guard + meta-test), backend/tests/test_p3_onboarding.py (+ FX1 regression), frontend/tests/viewport.regression.test.ts (+ FX2/FX3/FX6/FX7 checks). Docs: FINARODA_SAAS_PRD.md (F3b), FINARODA_SAAS_SPEC.md (history endpoints), FINARODA_SAAS_UX.md (fix-round bullet), ATP.md, CHANGELOG.md, VERSIONS.md, SESSION_HANDOFF.md.
- DB CHANGES: none.
- CONFIG ADDED: none.
- VALIDATION: pytest **185/185** (was 182; +3: forbidden-term guard + meta-test + FX1 new-user regression) in the project venv; tsc clean; eslint clean; frontend unit **62/62** (was 58; +4 FX structural checks). No em-dash, no SL/TP/ENTRY in UI copy, red-line suites untouched and green.
...
- STOP POINTS (all checked, none halted, all reported):
  - **S1 (Stripe cannot express "first charge only" in ILS): cleared.** Stripe Coupon `duration=once` scopes the discount to the first charge; ILS is fully supported (amount_off in agorot + currency ils, or percent_off). No workaround needed.
  - **S2 (customer balance vs Israeli tax-doc logic): resolved by D-S8, no improvisation.** A credited/skipped month whose invoice total is 0 issues NO tax document (document only on amount>0 invoices); an internal audit row + admin visibility replace it. Partial-amount invoices (balance covers part) still have amount>0 and follow the normal document path unchanged.
  - **S3 (reshaping 003/004 risks dev data): cleared.** The coupons/coupon_applications/referrals tables were confirmed EMPTY on dev (0 rows) and dormant (no live reads/writes); mig 036 copies any rows it finds (non-destructive) before reshaping. No data at risk.
- SOLUTION:
  - **DB (mig 036, non-destructive reshape of the dormant model):** `coupons` rebuilt into a Stripe mirror (stripe_coupon_id, stripe_promotion_code_id, discount_type percent|fixed, percent_off / amount_off_agorot, duration once, plan_restriction, max_redemptions, redeemed_count, expires_at, active); `coupon_applications` -> `coupon_redemptions` (redemption ledger synced from webhooks); `referrals` rebuilt into the binding+reward model (status bound|rewarded|void, reward_type balance_credit|banked, reward_amount_agorot, stripe_balance_transaction_id, UNIQUE referred_id); new `referral_credits` banked-credit ledger; subscription_events CHECK extended with the Stage-4 audit types (referral reward/credit/void, coupon_redeemed, zero_amount_invoice_no_document).
  - **core/coupon_service.py (new):** create_coupon drives Stripe Coupon + Promotion Code (DEV = deterministic fake ids, zero network) + mirror row; validate_coupon_for_plan (NOT_FOUND/INACTIVE/EXPIRED/MAX_REDEEMED/WRONG_PLAN); deactivate_coupon (flips the Stripe promo inactive + our active=0); record_redemption (idempotent per coupon,user + count sync).
  - **core/referral_service.py (new):** get_or_create_code (permanent 8-char code); bind_referral (once, immutable, self-referral blocked by id + email; silent no-op so signup never breaks); grant_reward_for_referred (bound->rewarded gate = idempotent; balance credit for a paying Stripe referrer, else banked); apply_banked_credits (on the referrer's first paid checkout, resolved to the plan they bought, credits stack); void_referral (removes a banked credit or posts a compensating balance transaction, audited); get_summary (invite card).
  - **Webhook wiring (stripe_service):** reward trigger on the referred user's first amount>0 invoice.paid (a 100%-coupon month is amount 0 and does NOT trigger); banked-credit application on checkout.session.completed; coupon-redemption sync from the completed session; zero-amount invoices skip the tax document (audit row instead). Checkout accepts an optional promotion_code, validated our-side for the plan before the session is created (applied via session `discounts`); with no code the session enables Stripe's hosted promotion-code field (allow_promotion_codes).
  - **C2 (webhook verify):** `verify_and_parse` now uses `stripe.Webhook.construct_event` (the SDK's t=,v1= HMAC + 300s tolerance, pure crypto, no network) then re-parses the body to a plain dict so downstream handlers are unchanged. The hand-rolled HMAC was removed. Test parity preserved (events are signed with the same scheme).
  - **API:** admin_promotions router (`/api/admin/coupons` create/list/deactivate, `/api/admin/referrals` list + void; 403 + audited); user `/api/referral` summary; `/api/billing/coupon/validate`; checkout promotion_code param; referral binding at magic-link + Google signup. Admin user-table `referrals` column now returns live counts (D-S10; was placeholder 0).
  - **Emails (D-S9):** referral reward earned (immediate or banked) + banked credit applied, product-email category. No em-dash.
  - **Frontend:** admin Coupons + Referrals sections (`components/admin/PromotionsAdmin.tsx`, wired into the console); `/r/[code]` route stores the code (localStorage) + redirects to login; login sends the stored code at signup; profile invite card (share link, copy, referral/reward counts); `lib/app/promotions.ts` pure helpers + api client additions.
- FILES CREATED: backend/migrations/036_stage4_coupons_referral.py, backend/core/coupon_service.py, backend/core/referral_service.py, backend/api/admin_promotions.py, backend/api/referral.py, backend/tests/test_stage4_coupons_referral.py, frontend/src/lib/app/promotions.ts, frontend/src/components/admin/PromotionsAdmin.tsx, frontend/src/app/r/[code]/page.tsx, frontend/tests/promotions.unit.test.ts.
- FILES MODIFIED: backend/core/stripe_service.py (SDK verify + webhook wiring + checkout promo), core/email.py (2 referral flows), models/billing.py, models/admin.py, models/auth.py, api/billing.py, api/auth.py, api/admin.py (live referral count), main.py (routers), tests/test_smoke.py (EXPECTED_TABLES); frontend lib/api.ts, app/(admin)/admin/page.tsx, app/(auth)/login/page.tsx, app/(profile)/profile/page.tsx, lib/version.ts.
- DB CHANGES: mig 036 (non-destructive reshape of empty dormant tables + new referral_credits + subscription_events CHECK extension).
- CONFIG ADDED: none (reuses the Stage-3R Stripe env; no new env vars).
- VALIDATION: pytest **146/146** (was 128; +18 TC-S4), tsc clean, eslint clean (next lint), frontend unit **58/58** (was 51; +7 promotions), em-dash guard clean. All Stripe calls mocked, zero network (AC8). Zero XP writes in new paths (AC8, asserted).
...
## [UPGRADE-STAGE3R / PSP migration Cardcom -> Stripe] - 2026-07-14
- GOAL: Replace the payment provider from Cardcom to Stripe (Stripe Checkout + Stripe Billing), keeping everything PSP-agnostic. Delete the homegrown recurring/dunning engine (Stripe Billing + Smart Retries own it). Keep the internal billing state machine as the single source of truth for entitlements, now fed only by Stripe webhooks. Add an Israeli tax-invoice provider layer (MOCK default + documented interface). OUT OF SCOPE: Stripe Customer Portal (D-R5), coupons/referral (Stage 4, redesigned Stripe-native), price changes.
- STOP POINTS (all checked, none halted, all reported):
  - **S1 (ILS unsupported): cleared.** Stripe fully supports ILS for Checkout, Prices, Subscriptions and Invoices.
  - **S2 (status mapping needs a new/removed state): cleared.** Our six states (none/trial/active/past_due/cancelled/expired) cover every Stripe status we consume; no state added or removed. One matrix EDGE was added (active -> expired) for an involuntary customer.subscription.deleted that never passed through past_due (AC6 sanctions matrix updates). Full Stripe -> internal mapping documented in billing_state.py + stripe_service.py.
  - **S3 (deleting dunning breaks a Stage-5 notification test): cleared.** test_stage5_notifications imports only trial_ending_soon_task; the failure/recovery emails moved to the webhook, so user-facing behavior is preserved.
- SOLUTION:
  - **core/stripe_service.py** (new, replaces cardcom_service): hosted Checkout Session (DEV fallback = zero-network fake session, mirrors DEV_RETURN_MAGIC_LINK); webhook signature verification (Stripe t=,v1= HMAC-SHA256, hand-rolled so the verify boundary is SDK-free and testable offline) + event-id idempotency (processed_webhook_events) + dispatch of checkout.session.completed / invoice.paid / invoice.payment_failed / customer.subscription.updated / customer.subscription.deleted; cancel via Stripe cancel_at_period_end keeping end-of-period access + churn hook; start_trial + expire_trials + drop_cancelled_to_free kept (our non-Stripe states).
  - **core/invoice_provider.py** (new, replaces cardcom_invoice): provider-agnostic issue_document(charge) -> doc; MOCK default (offline, zero network); documented InvoiceProvider interface + stubs for Green Invoice / iCount / EZcount (INVOICE_PROVIDER); document type default tax_invoice_receipt (VAT-registered LTD). Stripe invoices are never Israeli tax documents.
  - **core/billing_state.py:** added ACTIVE -> EXPIRED edge; documented the Stripe webhook -> internal-state mapping. Entitlements logic unchanged.
  - **api/billing.py** (new, replaces api/cardcom.py) at /api/billing/{checkout,trial,webhook,status,cancel}; main.py rewired.
  - Cron slimmed: billing_batch_task = expire_trials + drop_cancelled_to_free (NO charge step). scripts/seed_stripe_prices.py (idempotent Stripe Product+Price seed, stores price ids in system_settings).
  - **DELETED (no dead code):** cardcom_service.py, cardcom_invoice.py, models/cardcom.py, api/cardcom.py, scripts/run_subscription_renewal.py, run_renewal_batch/charge_recurring/dunning ladder, the /api/cron/billing charge step, all CARDCOM_* env + FEATURE_CARDCOM_LIVE + DUNNING_RETRY_OFFSETS_HOURS.
  - **Frontend:** /api/cardcom/* -> /api/billing/* in lib/api.ts, BillingBanner, ChurnSurvey, scan page; checkout pages relabeled Stripe (success notes webhook-driven activation). BillingBanner/formatAgorotIls/isEntitled unit logic is PSP-agnostic and unchanged.
- FILES CREATED: backend/migrations/035_stripe_migration.py, backend/core/stripe_service.py, backend/core/invoice_provider.py, backend/models/billing.py, backend/api/billing.py, backend/scripts/seed_stripe_prices.py, backend/tests/test_stage3r_stripe.py.
- FILES DELETED: backend/core/cardcom_service.py, backend/core/cardcom_invoice.py, backend/models/cardcom.py, backend/api/cardcom.py, backend/scripts/run_subscription_renewal.py, backend/tests/test_stage3_billing.py.
- FILES MODIFIED: config.py, .env.example, requirements.txt (+stripe), main.py, api/cron.py, app/tasks/billing_tasks.py, core/billing_state.py, tests (conftest, test_p1_auth_billing, test_b1_gating, test_smoke), frontend (lib/api.ts, components/app/BillingBanner.tsx, components/app/ChurnSurvey.tsx, app/(scan)/scan/page.tsx, app/checkout/{page,success,cancelled}/page.tsx), frontend version.
- DB CHANGES: mig 035 (additive/non-destructive renames). users.cardcom_token -> stripe_customer_id (+ stripe_subscription_id, card_last4, card_expiry); payment_transactions.cardcom_tx_id -> stripe_reference, cardcom_response_json -> provider_response_json; billing_documents.cardcom_document_id -> provider_document_id; new processed_webhook_events; billing_document_type default receipt -> tax_invoice_receipt.
- CONFIG ADDED: FEATURE_STRIPE_LIVE, STRIPE_SECRET_KEY, STRIPE_PUBLISHABLE_KEY, STRIPE_WEBHOOK_SECRET, INVOICE_PROVIDER, STRIPE_SUCCESS_PATH, STRIPE_CANCEL_PATH. CONFIG REMOVED: FEATURE_CARDCOM_LIVE, CARDCOM_* (6), DUNNING_RETRY_OFFSETS_HOURS.
- VALIDATION: pytest **128/128**, tsc clean, eslint clean (next lint), frontend unit **51/51**, shared **14/14**. All Stripe calls mocked, zero network (AC8). No em-dash in new UI copy. New baseline: 128 pytest (was 129; net of deleted Cardcom-specific cases + new TC-R3 cases).
