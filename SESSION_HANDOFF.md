...
## Where we are now
- **Active branch:** dev
- **Last commit (dev):** FIX-R3R4 (v0.10.0) — validations 3+4 fix round (11 bugs + Decisions A–E + UX). Backend `e2e49b8`, frontend `b852dd7`, docs commit on top.
- **Validation:** ✅ **pytest 71/71**, tsc clean, eslint clean, frontend unit 18/18, shared 14/14. ⚠ `next build` did NOT complete locally — Windows `.next/trace` EPERM lock then a >7min cold-build timeout (environment, not a code error; tsc already type-checks the whole project). Recommend a clean `pnpm build` on a fresh checkout before the dev→main merge.
- **main:** = `1338a26` (P2 scorer). Everything since (v0.4.2–v0.10.0) is **dev only** — Nadav merges to main manually.
