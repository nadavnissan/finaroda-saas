...
## Where we are now
- **Active branch:** dev
- **Last commit (dev):** RESPONSIVE-PASS (v0.10.1) — phone+desktop usability pass. Admin console made responsive, global overflow guards, tier-aware reveal copy, structural-lint viewport harness. Frontend-only + docs (on top of FIX-R3R4 v0.10.0).
- **Validation:** ✅ **pytest 71/71**, tsc clean, eslint clean (`next lint`), frontend unit **24/24** (18 prior + 6 viewport), shared 14/14. ⚠ `next build` still NOT verified locally — Windows `.next/trace` EPERM lock / cold-build timeout (environment, not a code error; tsc type-checks the whole project). Recommend a clean `pnpm build` on a fresh checkout before the dev→main merge.
- **main:** = `1338a26` (P2 scorer). Everything since (v0.4.2–v0.10.1) is **dev only** — Nadav merges to main manually.
