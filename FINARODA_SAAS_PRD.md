...
## F6 — Academy (וידאו למידה) — ✅ SHIPPED v0.9.0 (Package B B6, shell)
> **מומש v0.9.0 (B6):** מעטפת אקדמיה עם 12 מודולים = 12 ה-`academy` ids ב-`concept_tooltips_content.json`, כל מודול מרונדר את תוכן ה-`what` של המונחים שלו (אין שיעורים מומצאים). deep-link מ-Concept Tooltip (`/academy#<id>`) נוחת ומדגיש. +100 XP לשיעור **רק לתוכן-אמת (≥3 מונחים)** — 9 מודולים מזכים, 3 stubs=0. plan-gating + 2 מודולי בונוס לפי דרגה (אורתוגונלי לפלאן). ⬜ follow-up: וידאו YouTube בפועל (כרגע shell מבוסס-טקסט).
...
**AC:** AC1: גישה לפי פלאן (אדמין). AC2: נגן 9:16. AC3: צפייה נרשמת. AC4: השלמת שיעור מזכה +100 XP חד-פעמי לשיעור (`xp_events`, ראה SPEC §5.6); דרגות פותחות מודולי בונוס בלי לעקוף שערי פלאן.

## F7 — מנוי, Trial ותשלום — ✅ SHIPPED (trial spine v0.5.0 · Subscribe/B2 v0.8.0 · admin editor B7 v0.9.0)
> **מומש:** trial 14 יום ללא כרטיס (v0.5.0, TEST mode). עמוד ה-Subscribe (B2, v0.8.0) — 4 עמודות Free/Basic/Advanced/Pro, **טבלת השוואה חובה** (TC-J-002, Free ראשון+תמיד גלוי), D1 no-card trial CTA + 3 trust shields, "same engine, same threshold", "Continue on Free"; מחירים/מטבעות מ-`system_settings` דרך `GET /api/plans`. עורך ה-settings (‏`system_settings`) ב-B7 (v0.9.0). ⬜ follow-up: Cardcom live (credentials + `FEATURE_CARDCOM_LIVE=true`), אכיפת מגבלת Free 1-scan/day (כרגע coins/scan + chart_layers hard-gated בלבד), קופונים/referral.
...
## F11 — נוטיפיקציות + ברודקאסט — ✅ SHIPPED v0.9.0 (Package B B7)
> **מומש v0.9.0 (B7):** ברודקאסט אדמין (audience all/plan/trial-ending + channel) → banner in-app (‏`/api/broadcasts/active`, לעולם לא מכסה SCAN/disclaimer). `notifications_log` (mig 028) רושם את שני ה-system sends המוחלטים בלבד: day-11 trial reminder + journal-reveal teaser. ⬜ follow-up: שליחת מייל בפועל = stub לוגי.
...
- **email stubs** — שליחות המייל (ticket reply, broadcast email, day-11 reminder) רשומות ל-log כ-stubs; ספק המייל (Resend) לא מחובר בפועל בהן.
...
- **Free 1-scan/day enforcement** — coins/scan + chart_layers hard-gated בשרת; מגבלת Free 1-scan/day מוצגת ב-entitlements אך לא נאכפת (תואם B1c "scan again always allowed").
