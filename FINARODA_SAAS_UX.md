...
> **Stage 3R (2026-07-14, v0.16.0): PSP switched from Cardcom to Stripe (Stripe Checkout + Billing). The prior Cardcom design is superseded. Israeli tax documents are issued by a provider-agnostic invoice layer (`INVOICE_PROVIDER`), not by Stripe.** From the user's side the journey is unchanged: prices are still shown as final ₪ including VAT, the trial stays card-free (our own trial, NOT a Stripe trial), and cancel keeps end-of-period access + the churn survey. What changed under the hood: **checkout is a hosted Stripe Checkout Session** (the card never touches our servers, redirect back to the app), **activation is webhook-only** (never the browser redirect), recurring charges + retries run on **Stripe Billing / Smart Retries** (our own failure/recovery/cancel emails still fire so the copy stays ours), and billing endpoints moved to **`/api/billing/*`** (checkout, trial, webhook, status, cancel; `POST /api/billing/cancel` for end-of-period cancel). Each successful charge still produces one Israeli tax document via the invoice provider (`INVOICE_PROVIDER`); Stripe's own invoices are NOT presented as Israeli tax documents.
...
- **פעמון נוטיפיקציות (F11, Stage 5) — ✅ מומש v0.11.0:** ב-identity block של ההמבורגר — כפתור פעמון 🔔 עם badge unread (server-authoritative, cap "9+"), פאנל נפתח מתחתיו (newest-first), פתיחה מסמנת את הפריטים הגלויים כנקראים; לחיצה על פריט עם `link_path` מנווטת. arrival sound/vibration רק כשהאפליקציה פתוחה ורק אחרי אינטראקציה ראשונה (‏autoplay policy), degradation חיננית ב-iOS (‏`navigator.vibrate` no-op). **Settings** מקבל 5 toggles: IN-APP / ARRIVAL SOUND / VIBRATION / PRODUCT EMAILS / UPDATE EMAILS (נשמרים ב-`notification_prefs`, חוצה-מכשירים). **Admin → Broadcast:** preview של audience + נמענים opted-in לאימייל + שלב confirm; כל update-email נושא unsubscribe. **RED LINE:** reveal-teaser ללא ערך תוצאה, אין פוש יזום לסחור.
  ...
  - **שדרוג v0.18.0 (FX4 + F16):**
    - **FX4 · בורר מטבע עם נעילות (show-the-door):** מטבעות שאינם ב-allowlist של הפלאן **נשארים גלויים** בבורר עם 🔒 + "Available on <plan>" (עמעום קל). הקשה על מטבע נעול מנתבת ל-`/subscribe`, לעולם לא toggle ולא שגיאה שקטה — עקבי עם שיעורים נעולים ונעילת POSITION. הבחירה האוטומטית מדלגת על מטבעות נעולים. Free=[LINK,AVAX], Basic=[LINK,AVAX,SOL,ADA,DOGE], Pro=הכל; Trial=Pro. אדמין: סקשן "Coin access" (checklist + Pro wildcard). היסטוריה לא מושפעת (גייטינג על סריקות חדשות בלבד).
    - **F16 · כרטיס Market Narrative:** כרטיס תיאורי דטרמיניסטי אחרי הטבעת/רשימה ולפני ה-Blueprint (גם ב-empty-state), שמתאר את תוצאת-הסריקה באחד מ-6 מצבים (spike-in-failed-regime / quiet / transition / pass / watch / daily-limit). **S6 (daily_limit) מרונדר בתוך מסך ה-429/מכסה — מסך אחד, לא שניים.** מונחי-מפתח עם Concept Tooltip; disclaimer בכל כרטיס; **Free=paid**. Copy מסומן **DRAFT** (chip), החלפת copy סופי = שינוי תוכן-בלבד עתידי. שפה **תיאורית בלבד**, לעולם לא ציווי (RED LINE §8.1).
